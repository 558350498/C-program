...
**距离函数**：平面欧氏距离（纽约城市范围约 50km×50km，曲率误差可忽略，直接用 `sqrt(dx²+dy²)` 即可）
...
5.  **距离算法**：使用平面欧氏距离（`dx²+dy²`），纽约城市尺度下经纬度差值小，平面近似精度足够，无需 Haversine。
...
2. **欧氏距离测试**：对比已知的纽约地标坐标差（如时代广场到中央公园），验证 KNN 排序正确

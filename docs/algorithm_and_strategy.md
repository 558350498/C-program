...
- hot/cold dropoff 分组报告：基于真实 replay outcome，而不是全局估算。
...
5. 下一步优先做轻量 tile/grid side table，而不是完整真实道路格点路径规划：先把 pickup/dropoff tile heat、冷区分数、机会成本等统计稳定下来。
...
3. 热区终点降低机会成本。
4. 热区路径和高峰时段增加拥堵成本。
5. 第一版只输出 `opportunity_adjustment` 估算，不参与 dispatch。
6. 后续再 replay 对比调价前后的服务率、平均成本和未服务订单。
